<template lang="">
  ...
  <div class="lg:col-span-3">
    <div
      class="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 h-[calc(100vh-164px)] flex flex-col relative"
    >
      <!-- Loading -->
      <div
        v-if="isLoading"
        class="flex justify-center items-center h-full absolute top-0 left-0 w-full bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 z-10"
      >
      ...
      <div class="flex-1 overflow-y-auto min-h-0 hide-scrollbar relative" ref="chatContainer">
        <div class="p-6 space-y-4 min-h-full flex flex-col justify-end">
          <!-- No messages -->
          <div
            class="text-center text-gray-400 flex flex-col items-center justify-center absolute inset-0 p-4"
            v-if="messages.length === 0 && !isLoading"
          >
            <svg
              class="w-16 h-16 mx-auto mb-4 text-orange-300"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <!-- <div v-for="i in 5" :key="i" class="text-item flex flex-col gap-4">
            <div class="p-4 bg-gray-700 rounded-lg self-end">
              <p class="text-white">what is a fish?</p>
            </div>

            <div class="p-4 bg-gray-700 rounded-lg self-start">
              <p class="text-white">
          ...
          <div v-for="(message, index) in messages" :key="index" class="flex flex-col gap-4">
            <!-- User message -->
            <div class="p-3 bg-gray-500 rounded-lg self-end">
              <p class="text-white">{{ questions[index] }}</p>
            </div>
            <!-- Assistant message -->
            <div class="p-[14px] bg-gray-700 rounded-lg self-start">
              <div
      ...
      <div class="p-6 border-t border-gray-700">
        <form class="flex gap-3" @submit.prevent="handleAsk">
          <input
            ...
            class="flex-1 bg-gray-700 border-2 border-gray-500 rounded-lg px-3 py-6 text-white focus:outline-none focus:border-orange-300 h-[36px]"
          />
        ...
        <p class="text-gray-500 text-xs mt-2" v-else>
          Game Selected: <span class="font-bold text-gray-300">{{ chosenGame?.name }}</span>
        </p>
      </div>
    </div>
  </div>
